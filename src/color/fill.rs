...
impl From<Color> for FillColor {
    fn from(c: Color) -> Self {
        FillColor::One { off: PColor::Color(c.into()) }
    }
}

impl From<(Color, Color)> for FillColor {
    fn from((a, b): (Color, Color)) -> Self {
        FillColor::Two { off: b.into(), on: a.into() }
    }
}
...
impl FromScript for FillColor {
    ...
    fn from_script(
        ...
        match value {
            ScriptValue::Integer(n) => if n <= 0xf {
                Ok(FillColor::One { off: PColor::Palette(n as usize) })
                } else {
                    Ok(FillColor::Two { off: PColor::Palette((n & 0xf) as usize), on: PColor::Palette((n >> 4) as usize) })
                }
            // ScriptValue::Unit => Ok(N9Color::Pen),
            _ => Err(InteropError::impossible_conversion(TypeId::of::<FillColor>())),
        }
    }
}
