...
pub(crate) fn plugin(app: &mut App) {
    app
        .init_asset_loader::<P8StateLoader>()
...
impl AssetLoader for P8StateLoader {
    ...
    async fn load(
        ...
        let cart = P8CartLoader::default().load(reader, settings, load_context).await?;
...
impl AssetLoader for PngStateLoader {
    ...
    async fn load(
        ...
        let cart = PngCartLoader::default().load(reader, settings, load_context).await?;
        to_state(cart, load_context)
    }
