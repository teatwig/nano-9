...
fn read_from_uniformat<R: io::BufRead>(filename: &str, buf: &mut R) -> Result<Cartridge, Error> {
    ...
    Ok(Cartridge {
           ...
           sfx: cartridge_sfx,
           format: CartridgeFormat::PikuseruFormat,
...
fn read_from_p8format<R: io::BufRead>(filename: &str, buf: &mut R) -> Result<Cartridge, Error> {
    ...
    // XXX: This does not need to allocate like this.
    for line in buf.lines() {
        let l = line.unwrap();
...
impl Cartridge {
    ...
    #[cfg(feature = "pik")]
    pub fn from_pikuseru_file(filename: &str) -> Result<Cartridge, Error> {
    ...
    #[cfg(feature = "pik")]
    pub fn from_dpikuseru_file(filename: &str) -> Result<Cartridge, Error> {
    ...
    #[cfg(feature = "pik")]
    pub fn from_dpikuseru_string(data: Vec<u8>) -> Result<Cartridge, Error> {
        let mut buf_reader = Cursor::new(data);
    ...
    #[cfg(feature = "pik")]
    pub fn save_in_pikuseru(&mut self, filename: &str, version: &str) {
        info!("[CARTRIDGE] [Cartridge] Save the modified cartridge in Pikuseru format {:?}", filename);
